-- 003_push_guard.sql
-- Fix #5: Add server-side timestamp guard to push_changes
-- Prevents stale clients from overwriting newer server data.
-- Rows are only updated if the server's current last_modified_at <= the client's last_pulled_at.
...
BEGIN
  ...
  LOOP
    ...
    WHERE entries.user_id = _uid
      AND entries.last_modified_at <= _since;
  END LOOP;
  ...
  LOOP
    ...
    WHERE seeds.user_id = _uid
      AND seeds.last_modified_at <= _since;
  END LOOP;
  ...
  LOOP
    ...
    WHERE plants.user_id = _uid
      AND plants.last_modified_at <= _since;
  END LOOP;
  ...
  LOOP
    ...
    WHERE achievement_records.user_id = _uid
      AND achievement_records.last_modified_at <= _since;
  END LOOP;
  ...
  LOOP
    ...
    ON CONFLICT (id) DO UPDATE SET
      ...
      tier = EXCLUDED.tier,
      last_modified_at = _now
    WHERE user_stats.user_id = _uid
      AND user_stats.last_modified_at <= _since;
  END LOOP;
