-- 004_use_client_timestamps.sql
-- Use client-provided last_modified_at instead of server NOW() to keep timestamps
-- consistent between local and remote, eliminating false conflict detection.
-- Also disables the auto-update trigger that would override client timestamps.

-- Drop the trigger that overwrites last_modified_at on every UPDATE
DROP TRIGGER IF EXISTS entries_last_modified ON entries;
DROP TRIGGER IF EXISTS entry_emotions_last_modified ON entry_emotions;
DROP TRIGGER IF EXISTS seeds_last_modified ON seeds;
DROP TRIGGER IF EXISTS plants_last_modified ON plants;
DROP TRIGGER IF EXISTS achievement_records_last_modified ON achievement_records;
DROP TRIGGER IF EXISTS user_stats_last_modified ON user_stats;
...
BEGIN
  ...
  LOOP
    ...
    VALUES ((_record->>'id')::UUID, _uid, (_record->>'seed_id')::UUID,
            ...
            (_record->>'last_growth_date')::DATE, _client_ts)
    ON CONFLICT (id) DO UPDATE SET
      ...
      last_growth_date = EXCLUDED.last_growth_date, last_modified_at = EXCLUDED.last_modified_at
